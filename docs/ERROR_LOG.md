...
| 2026-09-20 | Phase 2 official access closure | Final HTTP/1.1 retry to `nsearchives.nseindia.com/content/indices/ind_close_all_17042026.csv` timed out four times with 0 bytes received. Combined with prior NSE API 403 and NSE Indices HTML/200 results, hosted-runner access to the official P0 paths is not viable. | Primary P0 dataset remains unavailable from GitHub Actions. | Close repeated retries; issue #6 is now the single blocker. Require authorized/official data delivery or a licensed vendor route. |
